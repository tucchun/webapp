...
<div id="modal_msgCenterInfo" class="modal fade">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            <form class="form-horizontal form-tb clearfix" id="msgCenterInfo-form">
                ...
                <input type="hidden" name="residentBirthNumber" value="${residentaudit.residentBirthNumber}">
                <input type="hidden" name="menderId" value="${residentaudit.menderId}">
                <!-- 监护人1 监护人2 -->
                ...
                <div class="form-group">
                 ...
                 <div class="form-group">
                    ...
                    <div class="col-sm-8">
                    	<textarea class="form-control" placeholder="若审核不通过，须输入备注，说明不通过原因；审核通过，则无需填写！" id="residentAuditRemark"  name="residentAuditRemark"><c:if test="${residentaudit.residentResult ne 0}">${residentaudit.residentAuditRemark}</c:if></textarea>
                    </div>
                </div>
                <!-- 
       ...
       -->
                <div class="form-group">
                    <div class="operation">
                    <c:if test="${residentaudit.residentResult eq 0}">
                    <input type="button" class="btn btn-main" value="审核通过" id="js-submit"/>
                    <input type="button" class="btn btn-main" value="审核不通过" id="js-submit2"/>
                    </c:if>
                    </div>
                </div>
            </form>
...
require([
	...
	$self.find("#js-submit").on("click", function(e){
        if($self.find('#msgCenterInfo-form').valid()){
            common.post({
                url: '${ctx}/residentAudit/auditPass',
                data: $self.find("#msgCenterInfo-form").serialize(),
                modal: $self.find("#modal_msgCenterInfo"),
                dataType: "JSON"
            }).done(function(data){
	...
	$self.find("#js-submit2").on("click", function(e){
		 //如果审核不通过，必须有不通过原因
		 var residentGuradianNo2 = $self.find('#residentAuditRemark').val();
		 if(residentGuradianNo2 == null || residentGuradianNo2==''){
			 common.alert("备注不能为空!");
			 return false;
        ...
        if($self.find('#msgCenterInfo-form').valid()){
            common.post({
                url: '${ctx}/residentAudit/auditPassNo',
                data: $self.find("#msgCenterInfo-form").serialize(),
                modal: $self.find("#modal_msgCenterInfo"),
                dataType: "JSON"
            }).done(function(data){
	...
	$self.find('#msgCenterInfo-form').validate({
        rules:{
        	residentAuditRemark:{maxlength:1000}
        }
    });
});
</script>