...
<div class="wrap">
    <div class="tb-head">义诊管理&gt;&gt;<c:if test="${action eq 'create' }">新增</c:if><c:if test="${action eq 'update' }">修改</c:if>义诊</div>
        ...
        <form action="#"  id="js-form">
            ...
            <table class="table wrap-table">
                ...
                <tr>
                    ...
                    <td>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" name="activityPlace" placeholder="义诊地点" value="${serviceActivity.activityPlace }" />
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <c:if test="${action eq 'create' }">
	                        <div class="form-inline sort_wrap" data-optype="update">
	                            <input type="text" class="form-control" name="doctor_arr" placeholder="请输入医师姓名"/>&nbsp;&nbsp;<input type="text" class="form-control" style="width: 300px;" name="doctor_department_arr" placeholder="请输入该医师名头，XXX医院XXXX专家"/>
                        ...
                        <c:if test="${action eq 'update' }">
	                        <c:forEach items="${doctorList }" var="doc" varStatus="status">
								<div class="form-inline sort_wrap" data-optype="update">
									<input type="text" class="form-control" name="doctor_arr" value="${doc.doctor_name }">&nbsp;&nbsp;<input type="text" class="form-control" style="width: 300px;" name="doctor_department_arr" value="${doc.doctor_department }">
									<i class="icon icon-del"></i><c:if test="${status.index == doctorList.size()-1}"><i class="icon icon-add"></i></c:if>
								</div>
							</c:forEach>
                        </c:if>
                    </td>
...
<script>
    ...
    require(['jquery','js/components-select','js/newClinic','validate','message_zh'], function($,select,newClinic){
    	var $self = $("#newClinic");
		                     ...
		                     success: function(data){
		                         ...
		                         if(data.ret_code==1){
		                             common.createTab({
		                                 ...
		                                 key: 'clinicList',
		                                 callback: function(){
		                                     common.closeTab("newClinic",null,false);
		                                 }
		                             });
		                         }
		                     }
    	...
    	$self.find('#js-form').validate({
            rules:{
                ...
                doctor_arr:"required",
                doctor_department_arr:"required",
            ...
            messages:{
            	...
            	doctor_arr:{required:"请输入医师"},
            	doctor_department_arr:{required:"请输入医师名头"},
            	user_crowdArr:{required:"请选择参与人群"}
            }
        });
