...
<div class="wrap">
    <div class="tb-head">体检管理&gt;&gt;<c:if test="${action eq 'create' }">新增</c:if><c:if test="${action eq 'update' }">修改</c:if>体检</div>
        ...
        <form action="#"  id="js-form">
            ...
            <table class="table wrap-table">
                ...
                <tr>
                    ...
                    <td>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" name="subject" placeholder="主题" value="${serviceActivity.subject }" required />
                        </div>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" name="activityPlace" placeholder="体检地点" value="${serviceActivity.activityPlace }" />
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <c:if test="${action eq 'update' }">
	                        <c:forEach items="${doctorList }" var="doc" varStatus="status">
								<div class="form-inline sort_wrap" data-optype="update">
									<input type="text" class="form-control" name="doctor_arr" value="${doc.doctor_name }">
...
<script>
    ...
    require(['jquery','js/components-select','js/cascade','js/newExamination','validate','message_zh'], function($,select,cascade,newExamination){
    	var $self = $("#newEamination");
    	var sel = $self.find('#select-hecadre');
		var selected = new select(sel);
     	$self.find('.wrap-table').on('click','i.icon-add',function(){
     		newExamination.add($(this));
        });
        $self.find('.wrap-table').on('click','i.icon-del',function(){
        	newExamination.del($(this));
        });
     	...
     	$self.find("#js-submit").on("click", function(e){
    		...
    		if($self.find('#js-form').valid()){
    			common.confirm({
	     			...
	     			success:function(){
			    		common.fetch({
		                     url: '${ctx}/examinationList/${action}',
		                     type: 'POST',
		                     ...
		                     success: function(data){
		                         ...
		                         if(data.ret_code==1){
		                             common.createTab({
		                                 uri: '${ctx}/examinationList',
		                                 data: {name: '体检管理'},
		                                 key: 'examinationList',
		                                 callback: function(){
		                                     common.closeTab("newEamination",null,false);
		                                 }
		                             });
		                         }
		                     }
