...
<div class="wrap clinicList">
	<div class="tb-head">体检管理</div>
	<div class="tb-content">
		<div class="tb-wrap">
			<div class="opt clearfix">
				<div class="pull-right">
					<button class="btn btn-main" type="button" id="js-delete">删除</button>
					<button class="btn btn-main" type="button" id="js-create">新增体检</button>
				</div>
			</div>
			<table cellspacing="0"
				...
				<tbody>
					<c:forEach items="${pages.content}" var="dataList"
						varStatus="status">
						<tr>
			...
			<tags:pagination page="${pages}" action="${ctx}/examinationList" pageId="examinationList" />
		</div>
	</div>
</div>
...
require([ 'jquery', 'js/cascade' ], function($, cascade){
     	var $self = $("#examinationList");

     	$self.find("#js-create").on("click", function(e){
     		common.createTab({
     			uri: '${ctx}/examinationList/create',
     			data: {name: '新增体检'},
     			key: 'newEamination'
     		});
     	});
     	...
     	$self.find(".js-update").on("click", function(e){
     		...
     		common.createTab({
     			uri: '${ctx}/examinationList/update/'+id,
     			data: {name: '修改体检'},
     			key: 'newEamination'
     		});
     	});
     	...
     	$self.find(".js-view").on("click", function(e){
     		var id = $(this).attr('data-id');
     		common.createTab({
     			uri: '${ctx}/examinationList/view/'+id,
     			data: {name: '体检详情'},
					...
					function(e) {
						...
						if (strids == '') {
							...
							common.confirm({
					            content:"确认删除？",
					            success:function(){
					                $.get("${ctx}/examinationList/delete/" + strids,function(data){
