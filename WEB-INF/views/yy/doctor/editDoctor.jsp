...
<div class="wrap">
	<div class="tb-head">医生管理&gt;&gt;修改医生</div>
	<div class="tb-content">
		<div class="tb-wrap">
			<form id="editDoctor-js-form">
				<input type="hidden" name="diseaseids" id="diseaseids"/>
				<input type="hidden" name="id" id="id" value="${data.doctor.id}"/>
				<table class="table wrap-table">
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="form-inline col-sm-10">
									<input type="text" name="name" id="name" value="${data.doctor.name}" class="form-control"
										placeholder="医生姓名">
								</div>
							</td>
						</tr>
						<tr>
							...
							<td>
								<div class="col-sm-10 form-inline">
								<tags:selectbox list="${doctorGradeList}" name="grade" value="${data.doctor.grade}" addNull="true"/>
								</div>
							</td>
						</tr>
						<tr>
								...
								<div class="form-choices-selects clearfix" id="select-community">
									<div class="col-sm-3">
										<select class="form-control select-left" multiple>
											<%-- <c:forEach items="${data.noSelectDiseaseList }" var="dis"
												varStatus="status">
									...
									<div class="col-sm-3">
										<select class="form-control select-right" id="diseaseList"
											multiple>
										<c:forEach items="${data.selectDiseaseList }" var="dis"
												varStatus="status">
												<option value="${dis.id }" selected>${dis.name }</option>
											</c:forEach>	
										</select>
									</div>
									<input type="text" name="dis" id="dis" value="1" style="visibility:hidden;"/>
								</div>
							</td>
						</tr>
						<tr>
                            ...
                            <div class="imglist clearfix">
                            	<div class="pull-left"><img src="${data.doctor.img }" alt="" id="imgurl" class="form-img"><input type="hidden" id="img" name="img" value="${data.doctor.img }"><a href="#">删除</a></div>
                            </div>
						</tr>
						<tr>
							<td colspan="2">
	...
	require([ 'jquery', 'js/area', 'js/components-select','js/cascade','upload','validate' ,'message_zh'], function($, area,
		...
		$.getJSON("${ctx}/Doctor/getAreaHispital", function(data){
	        new cascade({
			    ...
			    "parentId": "parentid",
			    "defaultPrimary": "${data.doctor.province}",
			    "defaultSecondary": "${data.doctor.city}",
        	});
		    new cascade({
			    ...
			    "parentId": "parentid",
			    "defaultPrimary": "${data.doctor.city}",
			    "defaultSecondary": "${data.doctor.hospital}"
		    });
		    new cascade({
			    ...
			    "parentId": "parentid",
			    "defaultPrimary": "${data.doctor.onedepartment}",
			    "defaultSecondary": "${data.doctor.twodepartment}"
		    });
	    });
		
		$self.find("#js-submit").on("click", function(e){
			$self.find('#dis').val($self.find('#diseaseList').attr('data-selected'));
			if($self.find('#editDoctor-js-form').valid()){
			...
			common.fetch({
			    ...
			    type: 'POST',
			    cache: false,
			    data: new FormData($('#editDoctor-js-form')[0]),
			    processData: false,
			    contentType: false,
			    dataType:"text",
			    success: function(rt) {
			       if('success' == rt){
			    	   ...
			    	   //common.closeTab('editDoctor');
			    	   common.createTab({
			               uri: '${ctx}/Doctor',
			               data: {name: '医生列表'},
			               key: 'doctorList',
			               callback: function(){
			                   common.closeTab("editDoctor", null, false);
			               }
			           });
			       }else{
			    	   $obj.show();  
		 ...
		 var action = "${ctx}/upload/uploadFile",_self = $('#editDoctor'),button = _self.find('#choseFile'),interval;;
		 new AjaxUpload(button,{
	        ...
	        $self.find('#editDoctor-js-form').validate({
	            rules:{
