...
import Dialog from './goodsTagDialog';
import './goodsTag.css';
...
class GoodsTag extends React.Component{
    ...
    getTagList(condition){
        ...
        http({
            ...
            if(data.ret_code === 1){
                let pageNumber = Math.ceil(data.ret_data.total/10);
                this.setState({
    ...
    render(){
        return (
            <Container className="p20" title={'商品标签管理'}>
                <div className="goodsTaglist">
                    ...
                    <Table rowKey="tag_id" columns={this.columns} data={this.state.gridData} />
...
ReactDom.render(
    <GoodsTag />,
    document.getElementById('__goodsTag/goodsTag__')
);